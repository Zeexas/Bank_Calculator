let sumFormat = document.querySelectorAll('.sum-format');

for (let i = 0; i < sumFormat.length; i++) {
  sumFormat[i].addEventListener('keyup', function (event) {
    let n = parseInt(this.value.replace(/\D/g,''),10); // do not let enter non-digit characters
    sumFormat[i].value = n.toLocaleString('ru-Ru');
  }, false);
}
...
function depCalculate() {
  ...
  depSumInitial = parseInt(depSumInitial.replace(/\D/g,''),10);
...
function loanCalculate() {
  ...
  loanSumInitial = parseInt(loanSumInitial.replace(/\D/g,''),10);
  ...
  let datesArray = [loanStartDate.toLocaleDateString('en-En')];
  let daysArray = [];
  ...
  for (let i = 1; i <= loanPeriodMonths; i++) {
    ...
    if ((nextDate.getMonth() - (i % 12)) > nextDateCheck.getMonth()) {
      ...
      datesArray.push(lastDayMonthBefore(nextDate).toLocaleString('en-En'));     // add DATE to <Date array>
    } else {
      table.rows[i].cells.item(1).innerHTML = nextDate.toLocaleDateString('ru-Ru');
      datesArray.push(nextDate.toLocaleString('en-En'));
    }
